...
def admin_user(db):
    return User.objects.create_user(
        username="admin", password="adminpass", role=User.Roles.ADMIN
    )
...
def customer1(db):
    return User.objects.create_user(
        username="cust1", password="custpass1", role=User.Roles.CUSTOMER
    )
...
def customer2(db):
    return User.objects.create_user(
        username="cust2", password="custpass2", role=User.Roles.CUSTOMER
    )
